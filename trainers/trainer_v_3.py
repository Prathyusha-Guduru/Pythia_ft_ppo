...
class PolicyAndValueWrapper(nn.Module):
    ...
    def get_last_policy_param_sum(self):
        policy_params = list(self.policy.parameters())
        for p in reversed(policy_params):
            if p.requires_grad:
                return torch.sum(p.detach()).item()
        return None  # No trainable parameters found

    def get_last_value_param_sum(self):
        value_params = list(self.value_model.parameters())
        for p in reversed(value_params):
            if p.requires_grad:
                return torch.sum(p.detach()).item()
        return None  # No trainable parameters found
    
    def reset_value_head(self):
        for name, module in self.value_model.named_modules():
            if any(k in name for k in ['score', 'classifier', 'value_head']):
                if isinstance(module, nn.Linear):
                    nn.init.zeros_(module.weight)
                    if module.bias is not None:
                        print("BIAS IS NOT NONEEEE!!!!!!")
                        nn.init.zeros_(module.bias)
                print(f"✅ Reset value head: {name}")
...
def get_param_sum(model):
    return sum(p.detach().float().sum().item() for p in model.parameters() if p.requires_grad)
...
class PPOTrainer(Trainer):
    ...
    def save_model(self, output_dir: Optional[str] = None, _internal_call: bool = False):
        ...
        model.policy.save_pretrained(policy_dir)

        if self.tokenizer is not None:
            self.tokenizer.save_pretrained(policy_dir)

        value_dir = os.path.join(output_dir, "value_model")
        os.makedirs(value_dir, exist_ok=True)
        torch.save(model.value_model.state_dict(), os.path.join(value_dir, WEIGHTS_NAME))

        if hasattr(model.value_model, 'config'):
            model.value_model.config.save_pretrained(value_dir)
    ...
    def train(self):
        ...
        cliprate_hit = torch.zeros(stats_shape, device=device)
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    ) # value for all tokens, generated from backbone of value model, trained value model (unwrapped means its ready for inference)
            ...
            for ppo_epoch_idx in range(args.num_ppo_epochs):
                ...
                for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                    ...
                    for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                        # print("\nMicro batch count", micro_batch_start, "\n")
                        with accelerator.accumulate(model):
                            ...
                            # print("Value loss for v_pred and mb_return", vf_losses1)
                            # print("Value loss for vpredclipped and mb_return", vf_losses1)

                            # CLIP RATE FRACTION / shape: (batch, seq) → True where clipped
                            clipped = (vf_losses2 > vf_losses1)

                            # shape: (batch, seq) → True where valid (non-padding)
                            mask = ~padding_mask_p1[micro_batch_inds]

                            # Only count valid positions
                            clipped_masked = clipped & mask

                            # Per sample: was there ANY clipped value?
                            sample_has_clipped = clipped_masked.any(dim=1)

                            # Percentage of samples that had at least one clip
                            cliprate_percent = 100.0 * sample_has_clipped.float().sum() / sample_has_clipped.numel()
                            ...
                            # print("VF_coef", args.vf_coef) # VF_coef 0.1
                            ...
                            #     accelerator.print(
                            #         f"\n\n----------------------------------------------"
                            #         f"Value params sum: {before_value} -> {after_value} "
                            ...
                            with torch.no_grad():
                                ...
                                cliprate_hit[ppo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = cliprate_percent
            ...
            with torch.no_grad():
                ...
                metrics["val/cliprate_fraction"] = self.accelerator.gather_for_metrics(cliprate_hit).mean().item()
                ...
                td_errors_squared = self.accelerator.gather_for_metrics(td_error_stats) ** 2
                metrics["val/td_error_squared_mean"] = td_errors_squared.mean().item()
                        ...
                        if self.accelerator.is_main_process:
                            ...
                            for name, param in self.value_model.named_parameters():
                                if param.requires_grad:
                                    param_stats[f"param/{name}/mean"] = param.data.mean().item()
                                    param_stats[f"param/{name}/std"] = param.data.std().item()

                            wandb.log(param_stats, step=self.state.global_step)
            ...
            if args.num_sample_generations > 0 and (update - 1) % self.sample_generations_freq == 0: 
                checkpoint_number = update // self.sample_generations_freq
                print("\nENTERED CHECKPOINT NUMBER", checkpoint_number, "\n")
                if self.generated_once == False:
                    self.generate_completions(sampling=True)  # creates completions.csv
                if self.generated_once == True:
                    self.process_completions(checkpoint_number)

            # if args.num_sample_generations > 0 and (update - 1) % self.sample_generations_freq == 0: 
            #     checkpoint_number = update // self.sample_generations_freq
            #     print(f"\nENTERED CHECKPOINT NUMBER {checkpoint_number} [Rank {self.accelerator.local_process_index}]\n")

            #     if self.accelerator.is_main_process:
            #         print(f"[Rank {self.accelerator.local_process_index}] Starting completions...")
            #         if not self.generated_once:
            #             self.generate_completions(sampling=True)
            #             self.generated_once = True
            #         self.process_completions(checkpoint_number)
            #         print(f"[Rank {self.accelerator.local_process_index}] Finished completions and processing.")

            #     self.accelerator.wait_for_everyone()
            #     print(f"[Rank {self.accelerator.local_process_index}] Passed synchronization.")
    ...
    def generate_completions(self, sampling: bool = False):
        print(f"[Rank {self.accelerator.local_process_index}] Starting completions...")
        # print(f"[Rank {self.accelerator.rank}] GPU memory: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
        self.generated_once = True
        num_samples = 10 
        args = self.args
        processing_class = self.processing_class
        ...
        df = df.apply(self.compute_token_lengths, axis=1, result_type='expand')
    ...
    def compute_token_lengths(self, row):
        # Tokenize query (prompt) with add_special_tokens=False to avoid adding special tokens
        prompt_encoding = self.tokenizer(row['query'], return_tensors='pt', add_special_tokens=False)
        prompt_ids = prompt_encoding['input_ids'].squeeze(0)  # shape: [seq_len]

        # Tokenize model response with add_special_tokens=False to avoid adding special tokens
        response_encoding = self.tokenizer(row['model response'], return_tensors='pt', add_special_tokens=False)
        response_ids = response_encoding['input_ids'].squeeze(0)  # shape: [seq_len]

        # Convert tensor to list for saving
        prompt_ids_list = prompt_ids.tolist()  # Convert to list of integers
        response_ids_list = response_ids.tolist()  # Convert to list of integers

        return {
            'prompt_token_len': prompt_ids.shape[0],
            'response_token_len': response_ids.shape[0],
            'query': row['query'],  # Keep original query
            'model response': row['model response'],  # Keep original model response
            'prompt_ids': prompt_ids_list,  # New column for prompt token IDs (tensor)
            'response_ids': response_ids_list  # New column for response token IDs (tensor)
        }


    def process_completions(self, checkpoint_number: int):
        # Load results dataframe
        print(f"[Rank {self.accelerator.local_process_index}] Finished process_completions")            
        result_df = pd.read_csv(f"{self.args.output_dir}/completions.csv")
        eval_df = pd.read_csv("training_eval_data.csv", nrows=N_SAMPLES)

        print("\nresult_df", result_df.head(), "\n")
        result_df['prompt_ids'] = result_df['prompt_ids'].apply(lambda x: torch.tensor(eval(x)))  
        result_df['response_ids'] = result_df['response_ids'].apply(lambda x: torch.tensor(eval(x)))  
        result_df['pred_value_per_token'] = None  # New column for storing per-token MC values
        eval_df['prompt_ids'] = eval_df['prompt_ids'].apply(parse_tensor_string)
        eval_df['response_ids'] = eval_df['response_ids'].apply(parse_tensor_string)
        eval_df['pred_value_per_token'] = None 

        unwrapped_value_model = self.accelerator.unwrap_model(self.model).value_model
        device = next(unwrapped_value_model.parameters()).device  # Get the model device

        for i in tqdm(range(len(result_df)), desc="Processing completions"):
            prompt_ids = result_df.loc[i, 'prompt_ids']
            response_ids = result_df.loc[i, 'response_ids']
            context_length = len(prompt_ids)

            postprocessed_response = response_ids 
            postprocessed_query_response = torch.cat((prompt_ids, postprocessed_response), dim=0).unsqueeze(0).to(device)
            ...
            value = full_value[:, context_length - 1 : -1].squeeze(-1)  # Shape: [response_len]

            result_df.at[i, 'pred_value_per_token'] = value.tolist()

        for i in tqdm(range(len(eval_df)), desc="Processing completions for eval_df"):
            prompt_ids = eval_df.loc[i, 'prompt_ids']
            response_ids = eval_df.loc[i, 'response_ids']
            context_length = len(prompt_ids)
            ...
            eval_df.at[i, 'pred_value_per_token'] = value.tolist()

        eval_df_dir = os.path.join(self.args.output_dir, "eval_df")
        os.makedirs(eval_df_dir, exist_ok=True)
        eval_df.to_csv(f'{self.args.output_dir}/eval_df/eval_df_{checkpoint_number}.csv')
            
        # Save the results dataframe back to a CSV file
        result_df.to_csv(f'{self.args.output_dir}/processed_completions_{checkpoint_number}.csv')
